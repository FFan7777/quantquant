...
# ── 排序学习 & 时序样本权重 ────────────────────────────────────────────────
# RANKING_MODE=True:  用 rank:ndcg + 行业分桶 取代 reg:squarederror
#   优势：直接优化排序目标，不浪费容量拟合绝对百分位值；行业内竞争更精准
#   HP搜索仍用回归模式（快速代理），只有最终训练用 Ranking
# TIME_DECAY_ALPHA:   近期训练样本加权系数（2021年牛市样本权重约为2018年的1.65倍）
...
def load_tech_basic_features(conn) -> pd.DataFrame:
    ...
    df = conn.execute(f"""
        WITH dp AS (
            SELECT ts_code, trade_date, open, high, low, close, pre_close, pct_chg, vol, amount, adj_factor
            FROM daily_price
            WHERE trade_date >= '{DATA_START}' AND trade_date <= '{END_DATE}'
...
def preprocess_panel(panel: pd.DataFrame,
    ...
    # 不整表 copy；各截面内 process_group 返回修改后副本，原表不动
...
def train_xgb(train: pd.DataFrame, feature_cols: list,
    ...
    """
    训练 XGBoost 截面选股模型。
    RANKING_MODE=True  → rank:ndcg 行业分桶（调用 _train_xgb_ranking，忽略 sample_weight）
    RANKING_MODE=False → reg:squarederror，支持 sample_weight 时间衰减
    ...
    if RANKING_MODE and "industry" in train.columns:
        return _train_xgb_ranking(train, feature_cols, val, params, n_estimators)
...
def train_lgbm(train: pd.DataFrame, feature_cols: list,
               ...
               params: dict = None,
               n_estimators: int = 1000,
               sample_weight: np.ndarray = None) -> lgb.Booster:
    """
    训练 LightGBM 截面选股模型（regression + 时间衰减权重，与 XGBoost ensemble）。
    val:           外部验证集（用于 early stopping）
    params:        XGB 格式超参 dict，自动转换为 LGB 等价参数
    sample_weight: 样本权重数组（time-decay），None 时自动从 TIME_DECAY_ALPHA 计算
    """
    ...
    if sample_weight is None:
        sample_weight = compute_sample_weights(train, TIME_DECAY_ALPHA)
...
class _BoosterWrapper:
    """
    将 xgb.Booster（rank:ndcg 训练产物）包装为 XGBRegressor 兼容接口。
    - predict(X): 接受 numpy array，返回 ranking scores（相对顺序有意义）
    - save_model / feature_importances_ / best_iteration 均已适配
    - 保存格式与 XGBRegressor 完全兼容（可被 infer_today.py 正常加载）
    """
...
def compute_sample_weights(df: pd.DataFrame, alpha: float = 0.5) -> np.ndarray:
    """
    指数时间衰减样本权重：近期数据获得更高权重，缓解训练期与预测期 Regime 差异。
    alpha=0.5, 训练2018-2021: 2021年样本权重约为2018年的1.65倍（e^0.5 ≈ 1.65）。
    """
    if alpha <= 0.0:
    ...
    if max_y == min_y:
        return np.ones(len(df), dtype=np.float32)
    norm = (years - min_y) / (max_y - min_y)   # [0, 1]
    w = np.exp(alpha * norm)
...
def make_ranking_groups(df: pd.DataFrame) -> tuple:
    """
    为 rank:ndcg 训练创建行业分桶分组。
    按 (trade_date, industry) 分组，每组内的股票参与 NDCG 排序竞争。
    相比全市场排序（5000+只作为一组），行业内竞争更精准且避免过拟合。

    返回: (sorted_df, group_sizes)
      - sorted_df    : 按 (trade_date, industry) 排序（保证同组样本连续）
      - group_sizes  : list[int]，每组样本数，总和 == len(df)
    """
...
def _train_xgb_ranking(train: pd.DataFrame, feature_cols: list,
                        val: pd.DataFrame = None,
                        params: dict = None,
                        n_estimators: int = 1000) -> _BoosterWrapper:
    """
    XGBoost rank:ndcg 行业分桶排序训练。
    - 每个 (trade_date, industry) 组内最大化 NDCG@10
    - 自动应用 TIME_DECAY_ALPHA 时间衰减权重
    - 返回 _BoosterWrapper，接口与 XGBRegressor 完全兼容
    """
    p = {**_DEFAULT_XGB_PARAMS, **(params or {})}
    print(f"\n[训练] XGBoost rank:ndcg  depth={p['max_depth']}  "
          f"mcw={p['min_child_weight']}  λ={p['reg_lambda']}  "
          f"[RANKING+decay={TIME_DECAY_ALPHA}]")

    # 排序 & 分组（必须连续）
    train_sorted, group_sizes = make_ranking_groups(train)
    X_tr = train_sorted[feature_cols].values.astype(np.float32)
    # rank:ndcg 要求非负整数 relevance score（float 会报错）
    # label 是截面百分位 [0,1]，×100 后取整得到 [0,100] 整数，保留相对排序
    y_tr = (train_sorted["label"].values * 100).clip(0, 100).astype(np.int32)

    avg_g = np.mean(group_sizes)
    print(f"  训练集: {len(train_sorted):,} 行, "
          f"{train_sorted['trade_date'].nunique()} 截面, "
          f"{len(group_sizes)} 分组 (avg={avg_g:.1f} 只/组)")

    # rank:ndcg 的 weight 必须是每组一个（不是每行），代表该 query 的重要性
    # 用时间衰减对每个 (date, industry) 组加权（近期截面权重更高）
    grp_dates = (
    ...
    # rank:ndcg 的 ndcg 指标不敏感，不适合做 early stopping
    # 使用固定轮数；n_estimators 由调用方决定（HP搜索=300，最终训练=500）
    fixed_rounds = min(n_estimators, 500)
    if val is not None and len(val) > 0:
        print(f"  验证集: {val['trade_date'].nunique()} 截面（监控用，不做 ES）")
...
def quick_eval_xgb_on_val(train: pd.DataFrame, val: pd.DataFrame,
    ...
    val = val.copy()
    val["pred"] = model.predict(val[feature_cols].values.astype(np.float32))
    ic_list = []
...
def _compute_val_ic_for_model(model_xgb, model_lgb, val, feature_cols, w_xgb=0.5):
    ...
    if model_lgb is not None:
        ...
        val2 = val.copy()
        val2["pred"] = model_xgb.predict(val[feature_cols].values.astype(np.float32))
...
def run_wfo_predictions(panel: pd.DataFrame,
    ...
    for i, (win_start, win_end, train_cut) in enumerate(windows):
        ...
        if ENSEMBLE_LGB:
            ...
            X = pred_sub[feature_cols].values.astype(np.float32)
...
def main():
    ...
    if ENSEMBLE_LGB and lgb_train_only is not None:
        ...
        val2_lgb = val.copy()
        val2_lgb["pred"] = lgb_train_only.predict(val[avail_features].values.astype(np.float32))
        ...
        val_pred_df["pred"] = xgb_train_only.predict(
            val[avail_features].values.astype(np.float32))
    os.makedirs(f"{OUTPUT_DIR}/csv", exist_ok=True)
    ...
    if ENSEMBLE_LGB and lgb_model is not None:
        ...
        val2 = val.copy()
        val2["pred"] = lgb_model.predict(val[avail_features].values.astype(np.float32))
